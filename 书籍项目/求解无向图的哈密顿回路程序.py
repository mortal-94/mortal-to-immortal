...
4.没有两个节点 b 和 c 在路径中占据相同的位置。---   (¬H[1,b] V ¬H[1,c]) ∧ (¬H[2,b] V ¬H[2,c]) ..∧.. (¬H[n,b] V ¬H[n,c])
5.非连通节点 b 和 c 在路径中不能相邻。---          (¬H[1,b] V ¬H[2,c]) ∧ (¬H[2,b] V ¬H[3,c]) ..∧.. (¬H[n-1,b] V ¬H[n,c])
        ...
        for j in nodes:
            if i < j:
                for k in nodes:
                    ...
                    CNF.append([-int(H[k - 1, i - 1]), -int(H[k - 1, j - 1])])  # 没有两个节点 b c 在路径中占据相同的位置，这里的 b c 是 i j
